    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="bg-[#111520] border border-white/[0.06] rounded-xl p-4 animate-in fade-in duration-300">
            <div className="text-[15px] font-semibold text-[#8892b0]">{k.label}</div>
      ...
      <Card className="bg-[#111520] border-white/[0.06]">
        <CardHeader className="pb-3">
          <CardTitle className="text-[18px] font-bold text-[#f0f2ff] flex items-center gap-2">
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-white/[0.06] hover:bg-transparent">
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">الاسم</TableHead>
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">الدور</TableHead>
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">عملاء</TableHead>
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">تواصل</TableHead>
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">اجتماعات</TableHead>
                <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">تقفيل</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {team.tele.map((name) => {
                ...
                return (
                  <TableRow key={name} className="border-b border-white/[0.04] hover:bg-[#1c2234]/50">
                    <TableCell className="text-[15px] font-semibold text-[#f0f2ff]">{name}</TableCell>
                    <TableCell>
                      <Badge className="bg-[#6c63ff]/15 text-[#a8a3ff] text-[12px] font-bold border-0">تيلي</Badge>
                    </TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#8892b0]">{s.total}</TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#00d4aa]">{s.contacted}</TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#ffd166]">{s.meetings}</TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#00d4aa]">{s.closed}</TableCell>
                  </TableRow>
                )
              })}
              {team.sales.map((name) => {
                ...
                return (
                  <TableRow key={name} className="border-b border-white/[0.04] hover:bg-[#1c2234]/50">
                    <TableCell className="text-[15px] font-semibold text-[#f0f2ff]">{name}</TableCell>
                    <TableCell>
                      <Badge className="bg-[#00d4aa]/15 text-[#00d4aa] text-[12px] font-bold border-0">سيلز</Badge>
                    </TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#8892b0]">{s.total}</TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#00d4aa]">{s.contacted}</TableCell>
                    <TableCell className="text-[15px] font-semibold text-[#ffd166]">{s.meetings}</TableCell>
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Select value={filterTele} onValueChange={setFilterTele}>
          ...
          <SelectContent className="bg-[#111520] border-white/[0.08]">
            ...
            {team.tele.map((n) => <SelectItem key={n} value={n} className="text-[15px]">{n}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={filterSales} onValueChange={setFilterSales}>
          ...
          <SelectContent className="bg-[#111520] border-white/[0.08]">
            <SelectItem value="all" className="text-[15px]">الكل</SelectItem>
            {team.sales.map((n) => <SelectItem key={n} value={n} className="text-[15px]">{n}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          ...
          <SelectContent className="bg-[#111520] border-white/[0.08]">
            <SelectItem value="all" className="text-[15px]">الكل</SelectItem>
            {STATUSES.map((s) => <SelectItem key={s.key} value={s.key} className="text-[15px]">{s.label}</SelectItem>)}
          </SelectContent>
        </Select>
        <div className="relative flex-1 min-w-[180px]">
          ...
          <Input
            ...
            className="h-8 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] pr-8"
          />
        </div>
        {selected.length > 0 && (
          <div className="flex items-center gap-1.5">
            <Button onClick={handleBulkArchive} size="sm" className="h-8 text-[15px] font-bold bg-amber-500/15 text-amber-400 border-0 gap-1 cursor-pointer">
              <Archive size={12} /> أرشفة ({selected.length})
            </Button>
            <Button onClick={handleBulkDelete} size="sm" className="h-8 text-[15px] font-bold bg-red-500/15 text-red-400 border-0 gap-1 cursor-pointer">
      ...
      <Card className="bg-[#111520] border-white/[0.06]">
        <CardContent className="p-0">
            ...
            <Table>
              ...
              <TableBody>
                {filteredLeads.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-8 text-[#4a5280] text-[15px] font-semibold">لا يوجد عملاء</TableCell>
                  </TableRow>
                ) : (
                  filteredLeads.map((lead) => (
                    <TableRow key={lead.id} className={`border-b border-white/[0.04] ${selected.includes(lead.id) ? 'bg-[#6c63ff]/5' : 'hover:bg-[#1c2234]/50'}`}>
                      ...
                      <TableCell className="text-[15px] font-semibold text-[#f0f2ff]">{lead.customerName || '—'}</TableCell>
                      <TableCell className="text-[15px] font-semibold text-[#8892b0]">{lead.phone || '—'}</TableCell>
                      ...
                      <TableCell>
                        <Badge className="bg-[#6c63ff]/15 text-[#a8a3ff] text-[12px] font-bold border-0">
                          {STATUSES.find((s) => s.key === lead.status)?.label || lead.status}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge className="bg-[#00d4aa]/15 text-[#00d4aa] text-[12px] font-bold border-0">
                          {SALES_STATUSES.find((s) => s.key === lead.salesStatus)?.label || lead.salesStatus || '—'}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-[14px] text-[#4a5280]">{formatDate(lead.createdAt)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
          ...
          {filteredLeads.length > 0 && (
            <div className="border-t border-white/[0.06] px-4 py-2 text-[14px] font-medium text-[#4a5280]">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Badge className="bg-[#1c2234] text-[#8892b0] text-[13px] font-bold border-0">{filtered.length} مؤرشف</Badge>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-white/[0.06] hover:bg-transparent">
                  ...
                  <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">التاريخ</TableHead>
                  <TableHead className="text-right text-[15px] font-bold text-[#4a5280] w-[80px]">إجراء</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-[#4a5280] text-[15px] font-semibold">لا يوجد عملاء مؤرشفين</TableCell>
                  </TableRow>
                ) : (
                  filtered.map((lead) => (
                    <TableRow key={lead.id} className="border-b border-white/[0.04] hover:bg-[#1c2234]/50">
                      ...
                      <TableCell className="text-[15px] font-semibold text-[#a8a3ff]">{lead.tele || '—'}</TableCell>
                      <TableCell className="text-[15px] font-semibold text-[#00d4aa]">{lead.sales || '—'}</TableCell>
                      <TableCell className="text-[15px] font-semibold text-[#4a5280]">{lead.archivedBy || '—'}</TableCell>
                      <TableCell className="text-[14px] text-[#4a5280]">{formatDate(lead.archivedAt)}</TableCell>
                      <TableCell>
                        <Button
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <Input
              ...
              className="h-8 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] max-w-[200px]"
            />
            <Select value={newRole} onValueChange={(v: 'tele' | 'sales') => setNewRole(v)}>
              <SelectTrigger className="w-[100px] h-8 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#8892b0]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-[#111520] border-white/[0.08]">
                <SelectItem value="tele" className="text-[15px]">تيلي</SelectItem>
                <SelectItem value="sales" className="text-[15px]">سيلز</SelectItem>
              </SelectContent>
            </Select>
            <Button
              onClick={handleAdd}
              className="h-8 text-[15px] font-bold bg-[#6c63ff] hover:bg-[#5b54e6] text-white gap-1 cursor-pointer"
            >
              <Plus size={12} />
              إضافة
            </Button>
                ...
                {editingName === name ? (
                  <>
                    <Input
                      ...
                      className="h-7 text-[15px] bg-[#0a0d14] border-[#6c63ff]/30 text-[#f0f2ff] max-w-[150px]"
                      autoFocus
                    />
                  ...
                  <>
                    <div className="w-7 h-7 rounded-full bg-[#6c63ff]/15 flex items-center justify-center text-[14px] text-[#a8a3ff] font-bold shrink-0">
                      {name.slice(0, 2)}
                    </div>
                    <span className="text-[15px] font-bold text-[#f0f2ff] flex-1">{name}</span>
                    <button
            ...
            {team.sales.map((name) => (
              <div key={name} className="flex items-center gap-2 py-1.5 px-3 rounded-lg hover:bg-[#1c2234]/50 transition-colors">
                {editingName === name ? (
                  ...
                  <>
                    <div className="w-7 h-7 rounded-full bg-[#00d4aa]/15 flex items-center justify-center text-[14px] text-[#00d4aa] font-bold shrink-0">
                      {name.slice(0, 2)}
                    </div>
                    <span className="text-[15px] font-bold text-[#f0f2ff] flex-1">{name}</span>
                    <button
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          className="bg-[#6c63ff] hover:bg-[#5b54e6] text-white gap-1.5 text-[15px] font-bold h-9 cursor-pointer"
        >
          <UserPlus size={14} />
          إنشاء مستخدم
        </Button>
        ...
        <Card className="bg-[#111520] border-[#6c63ff]/20">
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[15px] font-semibold text-[#8892b0] mb-1 block">اسم المستخدم (للدخول)</label>
                <Input
                  placeholder="مثال: amira2024"
                  value={newUser.username}
                  onChange={(e) => setNewUser((p) => ({ ...p, username: e.target.value }))}
                  className="h-9 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff]"
                  dir="ltr"
                />
              </div>
              <div>
                <label className="text-[15px] font-semibold text-[#8892b0] mb-1 block">كلمة المرور</label>
                <div className="relative">
                  <Input
                    ...
                    className="h-9 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] pr-3 pl-9"
                    dir="ltr"
                  />
                  <button
                    type="button"
              ...
              <div>
                <label className="text-[15px] font-semibold text-[#8892b0] mb-1 block">الاسم المعروض</label>
                <Input
              ...
              <div>
                <label className="text-[15px] font-semibold text-[#8892b0] mb-1 block">الدور</label>
                <Select value={newUser.role} onValueChange={(v: 'tele' | 'sales' | 'admin') => setNewUser((p) => ({ ...p, role: v }))}>
                  <SelectTrigger className="h-9 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[#111520] border-white/[0.08]">
                    <SelectItem value="tele" className="text-[15px]">تيلي</SelectItem>
                    <SelectItem value="sales" className="text-[15px]">سيلز</SelectItem>
                    <SelectItem value="admin" className="text-[15px]">أدمن</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="flex items-center gap-2 mt-4">
              <Button
                ...
                disabled={saving}
                className="bg-[#00d4aa] hover:bg-[#00b894] text-[#0a0d14] gap-1.5 text-[15px] font-bold h-9 cursor-pointer"
              >
              ...
              <Button
                ...
                className="bg-[#1c2234] text-[#8892b0] hover:text-[#f0f2ff] text-[15px] font-bold h-9 border-0 cursor-pointer"
              >
                إلغاء
              </Button>
            </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                className="h-9 text-[15px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] max-w-[250px]"
                dir="ltr"
              />
              <Button
                onClick={handleResetPassword}
                disabled={resetSaving}
                className="bg-amber-500 hover:bg-amber-600 text-[#0a0d14] gap-1.5 text-[15px] font-bold h-9 cursor-pointer"
              >
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-12 text-[#4a5280]">
              ...
              <div className="text-[14px] font-medium mt-1">اضغط &quot;إنشاء مستخدم&quot; لإضافة مستخدم جديد</div>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-white/[0.06] hover:bg-transparent">
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">المستخدم</TableHead>
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">اسم الدخول</TableHead>
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">الدور</TableHead>
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">الحالة</TableHead>
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280]">آخر دخول</TableHead>
                    <TableHead className="text-right text-[15px] font-bold text-[#4a5280] w-[120px]">إجراءات</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {users.map((user) => (
                    <TableRow key={user.id} className={`border-b border-white/[0.04] ${!user.is_active ? 'opacity-50' : ''}`}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[14px] font-bold shrink-0 ${user.role === 'admin' ? 'bg-amber-500/15 text-amber-400' : user.role === 'tele' ? 'bg-[#6c63ff]/15 text-[#a8a3ff]' : 'bg-[#00d4aa]/15 text-[#00d4aa]'}`}>
                            {(user.display_name || user.username).slice(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <div className="text-[15px] font-bold text-[#f0f2ff]">{user.display_name || '—'}</div>
                            <div className="text-[12px] text-[#4a5280]">{user.created_at ? new Date(user.created_at).toLocaleDateString('ar-EG') : '—'}</div>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-[15px] font-semibold text-[#8892b0] font-mono" dir="ltr">{user.username}</TableCell>
                      <TableCell>
                        <Badge className={`text-[12px] font-bold border-0 ${roleColors[user.role] || 'bg-[#1c2234] text-[#8892b0]'}`}>
                          {roleLabels[user.role] || user.role}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge className={`text-[12px] font-bold border-0 ${user.is_active ? 'bg-[#00d4aa]/15 text-[#00d4aa]' : 'bg-red-500/15 text-red-400'}`}>
                          {user.is_active ? 'مفعّل' : 'معطّل'}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-[14px] text-[#4a5280]">
      ...
      <Card className="bg-[#111520] border-white/[0.06]">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between py-2">
            <span className="text-[15px] font-semibold text-[#8892b0]">الوضع الداكن</span>
            <button
              onClick={toggleTheme}
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-[15px] font-semibold text-[#8892b0]">الإشعارات</span>
            <Badge className="bg-[#00d4aa]/15 text-[#00d4aa] text-[12px] font-bold border-0">مفعّلة</Badge>
          </div>
          <div className="flex items-center justify-between py-2">
            <span className="text-[15px] font-semibold text-[#8892b0]">المزامنة الفورية</span>
            <Badge className="bg-[#00d4aa]/15 text-[#00d4aa] text-[12px] font-bold border-0">مفعّلة</Badge>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div>
        ...
        <p className="text-[15px] font-semibold text-[#8892b0] mt-0.5">إدارة شاملة للنظام والفريق والعملاء</p>
      </div>
      ...
      <Tabs value={adminTab} onValueChange={(v) => setAdminTab(v as AdminTab)}>
          ...
          {ADMIN_TABS.map((tab) => {
            ...
            return (
              <TabsTrigger
                key={tab.key}
                value={tab.key}
                className="text-[15px] font-bold data-[state=active]:bg-[#6c63ff]/15 data-[state=active]:text-[#a8a3ff] text-[#8892b0] gap-1 px-3 py-1.5"
              >
                <Icon size={12} />
        ...
        <TabsContent value="tele" className="mt-4">
          <Card className="bg-[#111520] border-white/[0.06] p-4">
            <p className="text-[15px] text-[#8892b0] text-center py-8">
