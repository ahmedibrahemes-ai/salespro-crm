    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[12px] text-[#8892b0] mt-0.5">إضافة عدة عملاء دفعة واحدة</p>
        </div>
        <div className="flex items-center gap-2">
          {submittedCount > 0 && (
            <Badge className="bg-[#00d4aa]/15 text-[#00d4aa] text-[11px] border-0 gap-1">
              <Check size={10} />
      ...
      {duplicatePhones.size > 0 && (
        <motion.div variants={itemVariants}>
          <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
            ...
            <div>
              <div className="text-[12px] font-semibold text-amber-400">أرقام مكررة</div>
        ...
        <CardContent className="p-0">
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-white/[0.06] hover:bg-transparent">
                  ...
                  <TableHead className="text-right text-[11px] text-[#4a5280]">رابط المتجر</TableHead>
              ...
              <TableBody>
                {rows.map((row, idx) => (
                  <TableRow key={row.id} className={`border-b border-white/[0.04] ${
                    ...
                    <TableCell className="w-[40px] text-[11px] text-[#4a5280]">{idx + 1}</TableCell>
                    <TableCell>
                      <Input
                        placeholder="رابط المتجر"
                        value={row.storeUrl}
                        onChange={(e) => updateRow(row.id, 'storeUrl', e.target.value)}
                        className="h-7 text-[11px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] w-[130px]"
                      />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Select value={row.customerType || 'business'} onValueChange={(v) => updateRow(row.id, 'customerType', v)}>
                        <SelectTrigger className="h-7 text-[11px] w-[100px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff]">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="bg-[#111520] border-white/[0.08]">
                          <SelectItem value="business" className="text-[11px]">تجاري</SelectItem>
                          <SelectItem value="individual" className="text-[11px]">فرد</SelectItem>
                          <SelectItem value="enterprise" className="text-[11px]">مؤسسة</SelectItem>
                        </SelectContent>
                      </Select>
                    </TableCell>
                    <TableCell>
                      <Input
                        ...
                        className="h-7 text-[11px] bg-[#0a0d14] border-white/[0.08] text-[#f0f2ff] w-[100px]"
                      />
                    </TableCell>
                    <TableCell>
          ...
          <div className="border-t border-white/[0.06] px-4 py-3 flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              className="h-7 text-[11px] bg-[#1c2234] text-[#8892b0] hover:text-[#f0f2ff] border-white/[0.06] gap-1 cursor-pointer"
            >
              <Plus size={12} />
              إضافة صف
            </Button>
