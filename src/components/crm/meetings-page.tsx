        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {lead.phone && (
                <div className="text-[12px] font-medium text-[#8892b0] mt-0.5 font-mono" dir="ltr">
                  {lead.phone}
                </div>
              )}
              ...
              <div className="flex items-center gap-3 mt-2">
                {lead.tele && (
                  <span className="flex items-center gap-1 text-[12px] font-medium text-[#6c63ff]">
                    <Users size={9} />
                    تيلي: {lead.tele}
                  </span>
                )}
                {lead.sales && (
                  <span className="flex items-center gap-1 text-[12px] font-medium text-[#00d4aa]">
                    <Users size={9} />
                    مبيعات: {lead.sales}
                  </span>
                )}
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] font-semibold text-[#8892b0] mt-0.5">متابعة جميع اجتماعات الفريق</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Select value={memberFilter} onValueChange={setMemberFilter}>
            <SelectTrigger className="w-[180px] h-8 bg-[#111520] border-white/[0.06] text-[13px] text-[#f0f2ff]">
              <SelectValue placeholder="كل الفريق" />
            </SelectTrigger>
            <SelectContent className="bg-[#111520] border-white/[0.06] max-h-64 overflow-y-auto">
              <SelectItem value="all" className="text-[13px] text-[#8892b0] focus:text-[#f0f2ff] focus:bg-[#1c2234]">
                كل الفريق
              </SelectItem>
              {allMembers.map((m) => (
                <SelectItem
                  key={m.name}
                  value={m.name}
                  className="text-[13px] text-[#8892b0] focus:text-[#f0f2ff] focus:bg-[#1c2234]"
                >
                  <span className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${m.role === 'تيلي' ? 'bg-[#6c63ff]' : 'bg-[#00d4aa]'}`} />
                    {m.name}
                    <span className="text-[12px] font-medium text-[#4a5280]">({m.role})</span>
                  </span>
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="flex-1 relative">
          ...
          <Input
            ...
            className="h-8 bg-[#111520] border-white/[0.06] text-[13px] text-[#f0f2ff] placeholder:text-[#4a5280] pr-9"
          />
        </div>
      </div>
      ...
      {meetingLeads.length === 0 ? (
        <Card className="bg-[#111520] border-white/[0.06]">
          <CardContent className="py-16 text-center">
            ...
            <div className="text-[13px] font-medium text-[#4a5280]">{emptyMessage}</div>
          </CardContent>
        </Card>
      ) : (
