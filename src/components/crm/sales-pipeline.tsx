...
function SortableDealCard({ lead, stageColor }: { lead: Lead; stageColor: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Time since creation */}
      <div className="text-[10px] text-[#4a5280] mt-0.5 flex items-center gap-1 mr-5">
        {lead.status === 'won' ? (
          <>
            <Check size={10} className="text-[#00d4aa]" />
      ...
      {lead.status === 'lost' && lead.lostReason && (
        <div className="flex items-center gap-1 mt-1 mr-5">
          ...
          <span className="text-[9px] text-[#ff4d4d]/70">{getLostReasonLabel(lead.lostReason)}</span>
        </div>
      )}
      ...
      {lead.probability > 0 && lead.probability < 100 && (
        <div className="flex items-center gap-1.5 mt-1.5 mr-5">
          ...
          <span className="text-[10px] text-[#8892b0] shrink-0">{lead.probability}%</span>
        </div>
      )}
    </div>
  )
    ...
    <div className="flex-1 min-w-[130px]">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-1.5 mt-0.5">
          <span className="text-[10px] opacity-80">{leads.length} deals</span>
          {stageValue > 0 && (
            <span className="text-[9px] opacity-60">· {formatCurrency(stageValue)}</span>
          )}
        </div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h2 className="text-[18px] font-bold text-[#f0f2ff]">Sales Pipeline</h2>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-[#111520] border border-white/[0.06] rounded-xl px-4 py-2.5">
            <div className="text-[10px] text-[#4a5280] uppercase tracking-wider">Pipeline Value</div>
            <div className="text-[16px] font-bold text-[#00d4aa] mt-0.5">
              {formatCurrency(totalPipelineValue)} <span className="text-[11px] text-[#4a5280]">EGP</span>
            </div>
          </div>
          <div className="bg-[#111520] border border-white/[0.06] rounded-xl px-4 py-2.5">
            <div className="text-[10px] text-[#4a5280] uppercase tracking-wider">Active Deals</div>
            <div className="text-[16px] font-bold text-[#f0f2ff] mt-0.5">{totalActiveDeals}</div>
          </div>
        </div>
      </div>
            ...
            <div className="bg-[#161b28] border-2 border-[#6c63ff] rounded-xl p-3 shadow-[0_8px_32px_rgba(0,0,0,.5)] opacity-95 rotate-1">
              ...
              {activeLead.probability > 0 && activeLead.probability < 100 && (
                <div className="flex items-center gap-1.5 mt-1.5 ml-5">
                  ...
                  <span className="text-[10px] text-[#8892b0] shrink-0">{activeLead.probability}%</span>
                </div>
              )}
            </div>
          )}
