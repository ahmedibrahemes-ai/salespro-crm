    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isCurrentUser && (
                        <span className="text-[10px] font-bold text-[#6c63ff] bg-[#6c63ff]/15 px-2 py-0.5 rounded-md">أنت</span>
                      )}
                    </div>
        ...
        <div className="bg-[#111520] border border-white/[0.06] rounded-[14px] p-5 text-center">
          <div className="flex items-center justify-center gap-2 text-[15px] font-semibold text-[#f0f2ff] mb-4">
            <Medal size={17} className="text-[#6c63ff]" />
            نقاطك وشاراتك
          </div>

          <div className="mb-2">
            <div className="text-[52px] font-black text-[#6c63ff] leading-none" style={{ fontFamily: 'Cairo' }}>
              {topMember?.points?.toLocaleString() ?? '1,240'}
            </div>
            <div className="text-[14px] text-[#8892b0] mt-1">نقطة</div>
          </div>

          <div className="text-[15px] text-[#00d4aa] font-semibold mb-1">
            ...
            {topBadges.length > 0 ? topBadges.map((badgeKey, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[10px] text-[#8892b0] leading-tight">{badgeKey}</div>
                </motion.div>
              )
            }) : (
                ...
                {Object.entries(BADGE_MAP)
                  ...
                  .map(([key, info]) => (
                    <div key={key} className="bg-[#0a0d14] border border-white/[0.04] rounded-lg px-2.5 py-1.5 text-center opacity-40">
                      ...
                      <div className="text-[9px] text-[#4a5280]">{key}</div>
                    </div>
                  ))}
              </div>
            </div>
