 ...
 *  This ensures we can find a phone in the DB regardless of how it was stored. */
function generatePhoneVariants(phone: string): string[] {
  ...
  if (norm) {
    variants.add(norm)
    // If normalized to +966XXXXXXXXX, generate reverse variants
    if (norm.startsWith('+966')) {
      const digits = norm.substring(4) // Remove +966 → 5XXXXXXXX
      variants.add(digits)              // 5XXXXXXXX
      variants.add('0' + digits)        // 05XXXXXXXX
      variants.add('966' + digits)      // 9665XXXXXXXX
      variants.add('00966' + digits)    // 009665XXXXXXXX
    }
    // If starts with 00966, also add +966 form
    if (norm.startsWith('00966')) {
      variants.add('+' + norm.substring(2)) // +966...
    }
  }
...
// ===== GET handler - Read leads (always uses anon/public client for reliable pagination) =====
export async function GET(request: NextRequest) {
  try {
    ...
    // For READ operations, always use the anon/public client.
    // The admin (service role) client can have inconsistent pagination behavior.
    // Since RLS SELECT policies use USING (true), anon client can read all data.
    ...
    const { createClient: createReadClient } = await import('@supabase/supabase-js')
    const client = createReadClient(SUPABASE_URL, ANON_KEY, {
...
// POST handler - Create lead, Bulk create, Bulk delete, Archive, Unarchive, Rename team member
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'bulkCreate': {
        ...
        // Check for duplicates (INFO only — don't filter/remove any leads)
        // The client already handles duplicate filtering with a confirmation dialog.
        // The server just logs warnings for monitoring.
        const duplicateWarnings: Array<{ index: number; phone: string; reason: string }> = []
        const seenPhones = new Map<string, number>() // normalized phone → first index
        ...
        leads.forEach((lead, idx) => {
          ...
          if (seenPhones.has(norm)) {
            duplicateWarnings.push({
              index: idx,
              phone: norm,
              reason: `مكرر داخل المجموعة (صف ${seenPhones.get(norm)! + 1})`,
            })
            return
          }
      ...
      case 'checkDuplicatePhones': {
        ...
        // Query the database for any existing leads with these phone numbers
        // Generate ALL format variants for each input phone to find matches
        // regardless of how the phone is stored in the DB
        ...
        const searchPhonesArray = [...allSearchPhones]
        ...
        for (let i = 0; i < searchPhonesArray.length; i += BATCH_SIZE) {
          ...
          if (findErr) {
            ...
            // Non-fatal: return empty rather than failing the whole operation
            continue
          }
        ...
        for (const lead of existingLeads) {
          ...
          if (!norm) continue
          // Only report if this normalized phone was in our query
          if (!normalizedToOriginal[norm]) continue
          // Only report the first (oldest) match per normalized phone
          if (duplicates[norm]) {
