import { createClient } from '@supabase/supabase-js'

const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://gopgmisvyvqdbgkfekuf.supabase.co'
const SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdvcGdtaXN2eXZxZGJna2Zla3VmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg4MDcwNjksImV4cCI6MjA5NDM4MzA2OX0.TBj7jDZ5lCLMJcoXZOoWY7-RXBIa47FKeS1lkjqWUgA'

export const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  auth: {
    persistSession: true,
    autoRefreshToken: true,
  },
})

// ===== Server-side API helper =====
// Routes write operations through our Next.js API route which uses
// the Supabase service role key to bypass Row Level Security (RLS).
// Falls back to authenticated user token if service role key is unavailable.
async function serverOp<T = unknown>(operation: string, data: unknown): Promise<T> {
  // Get the current user's access token to pass to the server API.
  // This allows the server to use authenticated Supabase client as fallback
  // when the service role key is not configured.
  let authToken: string | undefined
  try {
    const { data: sessionData } = await supabase.auth.getSession()
    authToken = sessionData.session?.access_token || undefined
  } catch {
    // Session might not be available yet
  }

  const headers: Record<string, string> = { 'Content-Type': 'application/json' }
  if (authToken) {
    headers['X-Supabase-Auth'] = authToken
  }

  const res = await fetch('/api/leads', {
    method: 'POST',
    headers,
    body: JSON.stringify({ operation, data }),
  })
  const json = await res.json()
  if (!res.ok || json.error) {
    throw new Error(json.error || `Server operation "${operation}" failed (${res.status})`)
  }
  return json.data ?? json
}

// ===== Types =====
export interface TeamMember {
  id: string
  name: string
  role: 'tele' | 'sales' | 'admin'
  is_active: boolean
}

export interface LeadNote {
  id: string
  by: string
  cat: string
  text: string
  at: number
}

export interface Lead {
  id: string
  storeUrl: string
  phone: string
  customerName: string
  customerType: string
  brief: string
  contactResult: string
  contactResultAt: number | null
  tele: string
  sales: string | null
  meetingDate: string
  meetingTime: string
  meetingType: string
  meetingLink: string
  status: string
  salesStatus: string | null
  attended: string | null
  attendanceMarkedAt: number | null
  attendanceMarkedBy: string | null
  cancelledFrom: string | null
  cancelledAt: number | null
  createdAt: number
  assignedAt: number | null
  isArchived: boolean
  archivedAt: number | null
  archivedBy: string | null
  notes: LeadNote[]
}

// ===== DB Row Types =====
interface DbLead {
  id: string
  store_url: string | null
  phone: string | null
  customer_name: string | null
  customer_type: string | null
  brief: string | null
  contact_result: string | null
  contact_result_at: string | null
  tele_name: string | null
  sales_name: string | null
  meeting_date: string | null
  meeting_time: string | null
  meeting_type: string | null
  meeting_link: string | null
  status: string | null
  sales_status: string | null
  attended: string | null
  attendance_marked_at: string | null
  attendance_marked_by: string | null
  cancelled_from: string | null
  cancelled_at: string | null
  created_at: string | null
  assigned_at: string | null
  is_archived: boolean | null
  archived_at: string | null
  archived_by: string | null
}

interface DbNote {
  id: string
  by_name: string | null
  category: string | null
  text: string | null
  created_at: string | null
}

// ===== Helpers =====
/**
 * Safely parse a timestamp from DB. Handles both:
 * - ISO strings: "2026-06-03T10:00:00.000Z"
 * - Numeric strings (epoch ms): "1749000000000"
 * - null / undefined → null
 */
function safeTimestamp(val: string | number | null | undefined): number | null {
  if (val === null || val === undefined) return null
  if (typeof val === 'number') return val
  if (typeof val === 'string') {
    const trimmed = val.trim()
    if (!trimmed) return null
    // If it's a pure number (epoch ms), parse directly
    if (/^\d+$/.test(trimmed)) return parseInt(trimmed, 10)
    const d = new Date(trimmed)
    return isNaN(d.getTime()) ? null : d.getTime()
  }
  return null
}

function safeDate(val: string | null | undefined): string | null {
  if (!val || typeof val !== 'string') return null
  const trimmed = val.trim()
  if (!trimmed) return null
  if (/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) return trimmed
  const d = new Date(trimmed)
  if (isNaN(d.getTime())) return null
  return d.toISOString().split('T')[0]
}

function safeTime(val: string | null | undefined): string | null {
  if (!val || typeof val !== 'string') return null
  const trimmed = val.trim()
  if (!trimmed) return null
  if (/^\d{1,2}:\d{2}(:\d{2})?$/.test(trimmed)) return trimmed
  return null
}

/**
 * Normalize the `attended` field from DB:
 * - Old code stored boolean `true`/`false` which became `'true'`/`'false'` strings after column type change.
 * - New code uses `'attended'`/`'no-show'`/`null`.
 * This function converts legacy values to the new format.
 */
export function normalizeAttended(val: string | boolean | null | undefined): string | null {
  if (val === null || val === undefined) return null
  if (typeof val === 'boolean') return val ? 'attended' : 'no-show'
  const str = String(val).trim().toLowerCase()
  if (str === 'true' || str === 'attended') return 'attended'
  if (str === 'false' || str === 'no-show') return 'no-show'
  if (str === 'pending') return null
  return str || null
}

function leadFromDb(row: DbLead): Lead {
  return {
    id: row.id,
    storeUrl: row.store_url || '',
    phone: row.phone || '',
    customerName: row.customer_name || '',
    customerType: row.customer_type || '',
    brief: row.brief || '',
    contactResult: row.contact_result || '',
    contactResultAt: safeTimestamp(row.contact_result_at),
    tele: (row.tele_name || '').trim(),
    sales: row.sales_name ? row.sales_name.trim() : null,
    meetingDate: row.meeting_date || '',
    meetingTime: row.meeting_time ? row.meeting_time.substring(0, 5) : '',
    meetingType: row.meeting_type || '',
    meetingLink: row.meeting_link || '',
    status: row.status || 'new',
    salesStatus: row.sales_status || null,
    attended: normalizeAttended(row.attended),
    attendanceMarkedAt: safeTimestamp(row.attendance_marked_at),
    attendanceMarkedBy: row.attendance_marked_by || null,
    cancelledFrom: row.cancelled_from || null,
    cancelledAt: safeTimestamp(row.cancelled_at),
    createdAt: safeTimestamp(row.created_at) ?? 0,
    assignedAt: safeTimestamp(row.assigned_at),
    isArchived: row.is_archived || false,
    archivedAt: safeTimestamp(row.archived_at),
    archivedBy: row.archived_by || null,
    notes: [],
  }
}

function notesFromDb(notes: DbNote[] | undefined): LeadNote[] {
  if (!notes || !Array.isArray(notes)) return []
  return notes
    .sort((a, b) => new Date(a.created_at || '').getTime() - new Date(b.created_at || '').getTime())
    .map((n) => ({
      id: String(n.id),
      by: n.by_name || '',
      cat: n.category || '',
      text: n.text || '',
      at: n.created_at ? new Date(n.created_at).getTime() : Date.now(),
    }))
}

// ===== API Functions =====

export async function apiLogin(email: string, password: string) {
  const { data, error } = await supabase.auth.signInWithPassword({ email, password })
  if (error) throw error
  return data
}

export async function apiLogout() {
  const { error } = await supabase.auth.signOut()
  if (error) throw error
}

export async function apiGetSession() {
  const { data, error } = await supabase.auth.getSession()
  if (error) throw error
  return data.session
}

export async function apiGetTeam() {
  const { data, error } = await supabase
    .from('team_members')
    .select('*')
    .eq('is_active', true)
    .order('name')
  if (error) throw error

  // If the table is empty (e.g., after DB reset), fall back to DEFAULT_TEAM
  // so the app remains functional even without seeded data.
  if (!data || data.length === 0) {
    const { DEFAULT_TEAM } = await import('@/lib/store')
    return DEFAULT_TEAM
  }

  const team: { tele: string[]; sales: string[]; admin: string[] } = { tele: [], sales: [], admin: [] }
  ;(data as TeamMember[]).forEach((m) => {
    if (team[m.role as keyof typeof team]) team[m.role as keyof typeof team].push(m.name)
  })
  return team
}

export async function apiGetLeads(includeArchived = false): Promise<Lead[]> {
  // Strategy 1: Try server-side API first (bypasses RLS via admin/auth client)
  try {
    const headers: Record<string, string> = { 'Content-Type': 'application/json' }
    // Get auth token for the request
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const authToken = sessionData.session?.access_token
      if (authToken) headers['X-Supabase-Auth'] = authToken
    } catch {
      // Ignore session errors
    }

    const params = new URLSearchParams()
    if (includeArchived) params.set('archived', 'true')

    const res = await fetch(`/api/leads?${params.toString()}`, { headers })
    if (res.ok) {
      const json = await res.json()
      if (json.data && Array.isArray(json.data)) {
        console.log(`[apiGetLeads] Server API returned ${json.data.length} leads (source: ${json.source || 'unknown'}, total: ${json.total || json.data.length})`)
        return json.data as Lead[]
      }
    }
    console.warn('[apiGetLeads] Server API failed, falling back to client-side')
  } catch (err) {
    console.warn('[apiGetLeads] Server API error, falling back to client-side:', err)
  }

  // Strategy 2: Client-side fallback (may be blocked by RLS)
  const PAGE_SIZE = 1000 // Supabase REST API default max rows per request
  let allData: DbLead[] = []
  let from = 0
  let hasMore = true

  while (hasMore) {
    let query = supabase
      .from('leads')
      .select('*')
      .order('id', { ascending: true })
      .range(from, from + PAGE_SIZE - 1)

    if (!includeArchived) {
      query = query.eq('is_archived', false)
    }

    const { data, error } = await query
    if (error) throw error

    if (data && data.length > 0) {
      allData = allData.concat(data as DbLead[])
      from += PAGE_SIZE
      hasMore = data.length === PAGE_SIZE
    } else {
      hasMore = false
    }
  }

  console.log(`[apiGetLeads] Client-side returned ${allData.length} leads`)
  return allData.map((row) => leadFromDb(row))
}

/**
 * One-time migration: fix legacy `attended` values in the database.
 * Old code stored `true`/`false` (boolean) which became `'true'`/`'false'` strings
 * after the column type was changed from boolean to text.
 * This converts them to the new format: `'attended'`/`'no-show'`.
 * Runs in the background after data load — non-blocking.
 */
export async function migrateAttendedValues(): Promise<{ migrated: number }> {
  // Find leads with legacy attended values
  const { data: trueRows, error: e1 } = await supabase
    .from('leads')
    .select('id')
    .eq('attended', 'true')

  const { data: falseRows, error: e2 } = await supabase
    .from('leads')
    .select('id')
    .eq('attended', 'false')

  if (e1 || e2) {
    console.error('Migration query error:', e1 || e2)
    return { migrated: 0 }
  }

  let migrated = 0

  // Update 'true' → 'attended' via bulk update (single request per 500 leads)
  if (trueRows && trueRows.length > 0) {
    const ids = trueRows.map((r) => r.id)
    try {
      await serverOp<unknown>('bulkUpdate', { ids, updates: { attended: 'attended' } })
      migrated += ids.length
      console.log(`✅ Migrated ${ids.length} leads: 'true' → 'attended'`)
    } catch (err) {
      console.error('Migration update error (true→attended):', err)
    }
  }

  // Update 'false' → 'no-show' via bulk update (single request per 500 leads)
  if (falseRows && falseRows.length > 0) {
    const ids = falseRows.map((r) => r.id)
    try {
      await serverOp<unknown>('bulkUpdate', { ids, updates: { attended: 'no-show' } })
      migrated += ids.length
      console.log(`✅ Migrated ${ids.length} leads: 'false' → 'no-show'`)
    } catch (err) {
      console.error('Migration update error (false→no-show):', err)
    }
  }

  return { migrated }
}

export async function apiGetArchivedLeads(): Promise<Lead[]> {
  // Strategy 1: Try server-side API first (bypasses RLS via admin/auth client)
  try {
    const headers: Record<string, string> = { 'Content-Type': 'application/json' }
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const authToken = sessionData.session?.access_token
      if (authToken) headers['X-Supabase-Auth'] = authToken
    } catch {
      // Ignore session errors
    }

    const res = await fetch('/api/leads?archived_only=true', { headers })
    if (res.ok) {
      const json = await res.json()
      if (json.data && Array.isArray(json.data)) {
        console.log(`[apiGetArchivedLeads] Server API returned ${json.data.length} leads`)
        return json.data as Lead[]
      }
    }
    console.warn('[apiGetArchivedLeads] Server API failed, falling back to client-side')
  } catch (err) {
    console.warn('[apiGetArchivedLeads] Server API error, falling back to client-side:', err)
  }

  // Strategy 2: Client-side fallback
  const PAGE_SIZE = 1000 // Supabase REST API default max rows per request
  let allData: DbLead[] = []
  let from = 0
  let hasMore = true

  while (hasMore) {
    const { data, error } = await supabase
      .from('leads')
      .select('*')
      .eq('is_archived', true)
      .order('archived_at', { ascending: false })
      .range(from, from + PAGE_SIZE - 1)

    if (error) throw error

    if (data && data.length > 0) {
      allData = allData.concat(data as DbLead[])
      from += PAGE_SIZE
      hasMore = data.length === PAGE_SIZE
    } else {
      hasMore = false
    }
  }

  console.log(`[apiGetArchivedLeads] Client-side returned ${allData.length} leads`)
  return allData.map((row) => leadFromDb(row))
}

function leadToDb(lead: Partial<Lead>) {
  const contactAt =
    lead.contactResultAt || (lead.contactResult ? Date.now() : null)
  return {
    store_url: lead.storeUrl || null,
    phone: lead.phone || null,
    customer_name: lead.customerName || null,
    customer_type: lead.customerType || null,
    brief: lead.brief || null,
    contact_result: lead.contactResult || null,
    contact_result_at: contactAt ? new Date(contactAt).toISOString() : null,
    tele_name: lead.tele || null,
    sales_name: lead.sales || null,
    meeting_date: safeDate(lead.meetingDate || null),
    meeting_time: safeTime(lead.meetingTime || null),
    meeting_type: lead.meetingType || null,
    meeting_link: lead.meetingLink || null,
    status: lead.status || 'new',
    sales_status: lead.salesStatus || null,
    attended: lead.attended === undefined ? null : lead.attended,
    attendance_marked_at: lead.attendanceMarkedAt ? new Date(lead.attendanceMarkedAt).toISOString() : null,
    attendance_marked_by: lead.attendanceMarkedBy || null,
    cancelled_from: lead.cancelledFrom || null,
    cancelled_at: lead.cancelledAt ? new Date(lead.cancelledAt).toISOString() : null,
    assigned_at: lead.assignedAt ? new Date(lead.assignedAt).toISOString() : null,
    is_archived: lead.isArchived || false,
    archived_at: lead.archivedAt ? new Date(lead.archivedAt).toISOString() : null,
    archived_by: lead.archivedBy || null,
  }
}

function partialLeadToDb(updates: Partial<Lead>): Record<string, unknown> {
  const dbData: Record<string, unknown> = {}

  if ('storeUrl' in updates) dbData.store_url = updates.storeUrl || null
  if ('phone' in updates) dbData.phone = updates.phone || null
  if ('customerName' in updates) dbData.customer_name = updates.customerName || null
  if ('customerType' in updates) dbData.customer_type = updates.customerType || null
  if ('brief' in updates) dbData.brief = updates.brief || null
  if ('contactResult' in updates) dbData.contact_result = updates.contactResult || null
  if ('contactResultAt' in updates) dbData.contact_result_at = updates.contactResultAt ? new Date(updates.contactResultAt).toISOString() : null
  if ('tele' in updates) dbData.tele_name = updates.tele || null
  if ('sales' in updates) dbData.sales_name = updates.sales || null
  if ('meetingDate' in updates) dbData.meeting_date = safeDate(updates.meetingDate || null)
  if ('meetingTime' in updates) dbData.meeting_time = safeTime(updates.meetingTime || null)
  if ('meetingType' in updates) dbData.meeting_type = updates.meetingType || null
  if ('meetingLink' in updates) dbData.meeting_link = updates.meetingLink || null
  if ('status' in updates) dbData.status = updates.status || 'new'
  if ('salesStatus' in updates) dbData.sales_status = updates.salesStatus || null
  if ('attended' in updates) dbData.attended = updates.attended === undefined ? null : updates.attended
  if ('attendanceMarkedAt' in updates) dbData.attendance_marked_at = updates.attendanceMarkedAt ? new Date(updates.attendanceMarkedAt).toISOString() : null
  if ('attendanceMarkedBy' in updates) dbData.attendance_marked_by = updates.attendanceMarkedBy || null
  if ('cancelledFrom' in updates) dbData.cancelled_from = updates.cancelledFrom || null
  if ('cancelledAt' in updates) dbData.cancelled_at = updates.cancelledAt ? new Date(updates.cancelledAt).toISOString() : null
  if ('assignedAt' in updates) dbData.assigned_at = updates.assignedAt ? new Date(updates.assignedAt).toISOString() : null
  if ('isArchived' in updates) dbData.is_archived = updates.isArchived || false
  if ('archivedAt' in updates) dbData.archived_at = updates.archivedAt ? new Date(updates.archivedAt).toISOString() : null
  if ('archivedBy' in updates) dbData.archived_by = updates.archivedBy || null

  return dbData
}

// ===== Write Operations (via server-side API to bypass RLS) =====

// ===== Server-side Duplicate Phone Check =====
export interface DuplicatePhoneInfo {
  existingId: number
  existingOwner: string
  normalizedPhone: string
}

/**
 * Check if any of the given phone numbers already exist in the database.
 * Uses server-side normalized matching to catch duplicates regardless of format.
 * Returns a map of normalized phone → DuplicatePhoneInfo for phones that exist.
 */
export async function apiCheckDuplicatePhones(phones: string[]): Promise<Record<string, DuplicatePhoneInfo>> {
  if (!Array.isArray(phones) || phones.length === 0) return {}
  try {
    const data = await serverOp<Record<string, DuplicatePhoneInfo>>('checkDuplicatePhones', phones)
    return data || {}
  } catch (err) {
    console.warn('[apiCheckDuplicatePhones] Server API failed:', err)
    return {}
  }
}

export async function apiCreateLead(lead: Partial<Lead>): Promise<Lead> {
  try {
    // Route through server API (uses service role key → bypasses RLS)
    const data = await serverOp<Lead>('create', lead)
    return data
  } catch (serverErr) {
    // Fallback: try direct Supabase call (may fail if RLS is enforced)
    console.warn('[apiCreateLead] Server API failed, trying direct Supabase:', serverErr)
    const dbData = leadToDb(lead)
    let { data, error } = await supabase.from('leads').insert(dbData).select().single()
    if (error && /contact_result_at/i.test(error.message || '')) {
      delete (dbData as Record<string, unknown>).contact_result_at
      const retry = await supabase.from('leads').insert(dbData).select().single()
      data = retry.data
      error = retry.error
    }
    if (error) {
      console.error('[apiCreateLead] Direct Supabase error:', error.message, error.details, error.hint)
      throw new Error(`فشل في إضافة الصف: ${error.message}`)
    }
    return leadFromDb(data as DbLead)
  }
}

export async function apiBulkCreateLeads(leadsArr: Partial<Lead>[]): Promise<Lead[]> {
  if (!Array.isArray(leadsArr) || leadsArr.length === 0) return []
  try {
    const data = await serverOp<Lead[]>('bulkCreate', leadsArr)
    if (!Array.isArray(data)) {
      console.error('[apiBulkCreateLeads] Server returned non-array:', typeof data, data)
      return []
    }
    if (data.length !== leadsArr.length) {
      console.warn(`[apiBulkCreateLeads] MISMATCH: sent ${leadsArr.length}, received ${data.length}`)
    } else {
      console.log(`[apiBulkCreateLeads] Sent ${leadsArr.length}, received ${data.length} ✓`)
    }
    return data
  } catch (serverErr) {
    console.warn('[apiBulkCreateLeads] Server API failed, trying direct Supabase:', serverErr)
    const BATCH_SIZE = 500
    let allCreated: Lead[] = []
    for (let i = 0; i < leadsArr.length; i += BATCH_SIZE) {
      const batch = leadsArr.slice(i, i + BATCH_SIZE)
      const dbData = batch.map(leadToDb)
      const { data, error } = await supabase.from('leads').insert(dbData).select()
      if (error) {
        console.error('[apiBulkCreateLeads] Direct Supabase error:', error.message)
        throw new Error(`فشل في رفع البيانات: ${error.message}`)
      }
      if (data) allCreated = allCreated.concat((data as DbLead[]).map(leadFromDb))
    }
    if (allCreated.length !== leadsArr.length) {
      console.warn(`[apiBulkCreateLeads] FALLBACK MISMATCH: sent ${leadsArr.length}, received ${allCreated.length}`)
    }
    return allCreated
  }
}

export async function apiUpdateLead(id: string, updates: Partial<Lead>): Promise<Lead | null> {
  try {
    const data = await serverOp<Lead | null>('update', { id, updates })
    return data
  } catch (serverErr) {
    console.warn('[apiUpdateLead] Server API failed, trying direct Supabase:', serverErr)
    const dbData = partialLeadToDb(updates)

    // Try update with select first
    const { data, error } = await supabase.from('leads').update(dbData).eq('id', id).select().single()

    if (error) {
      // If select fails (e.g. RLS), try update without select
      const { error: updateError } = await supabase.from('leads').update(dbData).eq('id', id)
      if (updateError) {
        console.error('[apiUpdateLead] Direct Supabase error:', updateError.message, updateError.details, updateError.hint)
        throw new Error(`فشل التحديث: ${updateError.message}`)
      }
      console.warn('[apiUpdateLead] Update succeeded but select failed (likely RLS)')
      return null
    }

    return leadFromDb(data as DbLead)
  }
}

export async function apiDeleteLead(id: string) {
  try {
    await serverOp('delete', id)
  } catch (serverErr) {
    console.warn('[apiDeleteLead] Server API failed, trying direct Supabase:', serverErr)
    const { error } = await supabase.from('leads').delete().eq('id', id)
    if (error) {
      console.error('[apiDeleteLead] Direct Supabase error:', error.message)
      throw new Error(`فشل الحذف: ${error.message}`)
    }
  }
}

export async function apiDeleteLeadsBulk(ids: string[]) {
  try {
    await serverOp('bulkDelete', ids)
  } catch (serverErr) {
    console.warn('[apiDeleteLeadsBulk] Server API failed, trying direct Supabase:', serverErr)
    const BATCH_SIZE = 100
    for (let i = 0; i < ids.length; i += BATCH_SIZE) {
      const batch = ids.slice(i, i + BATCH_SIZE)
      const { error } = await supabase.from('leads').delete().in('id', batch)
      if (error) {
        console.error('[apiDeleteLeadsBulk] Direct Supabase error:', error.message)
        throw new Error(`فشل الحذف: ${error.message}`)
      }
    }
  }
}

export async function apiArchiveLeads(ids: string[], byName: string) {
  if (!Array.isArray(ids) || ids.length === 0) return
  try {
    await serverOp('archive', { ids, archivedBy: byName })
  } catch (serverErr) {
    console.warn('[apiArchiveLeads] Server API failed, trying direct Supabase:', serverErr)
    const BATCH_SIZE = 100
    const archivedAt = new Date().toISOString()
    for (let i = 0; i < ids.length; i += BATCH_SIZE) {
      const batch = ids.slice(i, i + BATCH_SIZE)
      const { error } = await supabase
        .from('leads')
        .update({ is_archived: true, archived_at: archivedAt, archived_by: byName })
        .in('id', batch)
      if (error) {
        console.error('[apiArchiveLeads] Direct Supabase error:', error.message)
        throw new Error(`فشل الأرشفة: ${error.message}`)
      }
    }
  }
}

export async function apiUnarchiveLeads(ids: string[]) {
  if (!Array.isArray(ids) || ids.length === 0) return
  try {
    await serverOp('unarchive', ids)
  } catch (serverErr) {
    console.warn('[apiUnarchiveLeads] Server API failed, trying direct Supabase:', serverErr)
    const BATCH_SIZE = 100
    for (let i = 0; i < ids.length; i += BATCH_SIZE) {
      const batch = ids.slice(i, i + BATCH_SIZE)
      const { error } = await supabase
        .from('leads')
        .update({ is_archived: false, archived_at: null, archived_by: null })
        .in('id', batch)
      if (error) {
        console.error('[apiUnarchiveLeads] Direct Supabase error:', error.message)
        throw new Error(`فشل إلغاء الأرشفة: ${error.message}`)
      }
    }
  }
}

export async function apiGetLeadNotes(leadId: string): Promise<LeadNote[]> {
  const { data, error } = await supabase
    .from('lead_notes')
    .select('*')
    .eq('lead_id', leadId)
    .order('created_at', { ascending: true })
  if (error) throw error
  return notesFromDb(data as DbNote[])
}

export async function apiAddNote(leadId: string, by: string, cat: string, text: string) {
  try {
    const data = await serverOp('addNote', { leadId, by, cat, text })
    return data
  } catch (serverErr) {
    console.warn('[apiAddNote] Server API failed, trying direct Supabase:', serverErr)
    const { data, error } = await supabase
      .from('lead_notes')
      .insert({ lead_id: leadId, by_name: by, category: cat, text })
      .select()
      .single()
    if (error) {
      console.error('[apiAddNote] Direct Supabase error:', error.message)
      throw new Error(`فشل إضافة الملاحظة: ${error.message}`)
    }
    return data
  }
}

export async function apiDeleteNote(noteId: string | number) {
  try {
    await serverOp('deleteNote', noteId)
  } catch (serverErr) {
    console.warn('[apiDeleteNote] Server API failed, trying direct Supabase:', serverErr)
    const { error } = await supabase.from('lead_notes').delete().eq('id', noteId)
    if (error) {
      console.error('[apiDeleteNote] Direct Supabase error:', error.message)
      throw new Error(`فشل حذف الملاحظة: ${error.message}`)
    }
  }
}

export async function apiUpdateNote(noteId: string | number, updates: { text?: string; category?: string }) {
  try {
    const data = await serverOp('updateNote', { noteId, updates })
    return data
  } catch (serverErr) {
    console.warn('[apiUpdateNote] Server API failed, trying direct Supabase:', serverErr)
    const dbData: Record<string, unknown> = {}
    if ('text' in updates) dbData.text = updates.text
    if ('category' in updates) dbData.category = updates.category
    const { data, error } = await supabase.from('lead_notes').update(dbData).eq('id', noteId).select().single()
    if (error) {
      // Retry without select (RLS may block it)
      const { error: updateError } = await supabase.from('lead_notes').update(dbData).eq('id', noteId)
      if (updateError) {
        console.error('[apiUpdateNote] Direct Supabase error:', updateError.message)
        throw new Error(`فشل تحديث الملاحظة: ${updateError.message}`)
      }
      return null
    }
    return data
  }
}

export async function apiGetSetting(key: string) {
  const { data, error } = await supabase.from('settings').select('value').eq('key', key).single()
  if (error && error.code !== 'PGRST116') throw error
  return data?.value
}

export async function apiSaveSetting(key: string, value: unknown) {
  try {
    await serverOp('saveSetting', { key, value })
  } catch (serverErr) {
    console.warn('[apiSaveSetting] Server API failed, trying direct Supabase:', serverErr)
    const { error } = await supabase.from('settings').upsert({ key, value }, { onConflict: 'key' })
    if (error) {
      console.error('[apiSaveSetting] Direct Supabase error:', error.message)
      throw new Error(`فشل حفظ الإعدادات: ${error.message}`)
    }
  }
}

export async function apiAddTeamMember(name: string, role: string) {
  try {
    const data = await serverOp('addTeamMember', { name, role })
    return data
  } catch (serverErr) {
    console.warn('[apiAddTeamMember] Server API failed, trying direct Supabase:', serverErr)
    const { data: existing } = await supabase
      .from('team_members')
      .select('*')
      .eq('name', name)
      .eq('is_active', false)
      .maybeSingle()

    if (existing) {
      const { data, error } = await supabase
        .from('team_members')
        .update({ is_active: true, role })
        .eq('id', existing.id)
        .select()
        .single()
      if (error) {
        console.error('[apiAddTeamMember] Direct Supabase error:', error.message)
        throw new Error(`فشل إضافة عضو: ${error.message}`)
      }
      return data
    }

    const { data, error } = await supabase.from('team_members').insert({ name, role }).select().single()
    if (error) {
      console.error('[apiAddTeamMember] Direct Supabase error:', error.message)
      throw new Error(`فشل إضافة عضو: ${error.message}`)
    }
    return data
  }
}

export async function apiRemoveTeamMember(name: string) {
  try {
    await serverOp('removeTeamMember', name)
  } catch (serverErr) {
    console.warn('[apiRemoveTeamMember] Server API failed, trying direct Supabase:', serverErr)
    const { error } = await supabase.from('team_members').update({ is_active: false }).eq('name', name)
    if (error) {
      console.error('[apiRemoveTeamMember] Direct Supabase error:', error.message)
      throw new Error(`فشل إزالة عضو: ${error.message}`)
    }
  }
}

export async function apiRenameTeamMember(oldName: string, newName: string) {
  try {
    await serverOp('renameTeamMember', { oldName, newName })
  } catch (serverErr) {
    console.warn('[apiRenameTeamMember] Server API failed, trying direct Supabase:', serverErr)
    const { error: e1 } = await supabase.from('team_members').update({ name: newName }).eq('name', oldName)
    if (e1) {
      console.error('[apiRenameTeamMember] Direct Supabase error:', e1.message)
      throw new Error(`فشل إعادة تسمية: ${e1.message}`)
    }
    await supabase.from('leads').update({ tele_name: newName }).eq('tele_name', oldName)
    await supabase.from('leads').update({ sales_name: newName }).eq('sales_name', oldName)
  }
}

// ===== Server-side Duplicate Detection =====
export interface DuplicateInfo {
  originalId: number
  originalTele: string
  originalSales: string | null
  duplicateIds: number[]
}

export interface DuplicatesResponse {
  duplicates: Record<string, DuplicateInfo>
  totalLeads: number
  totalDuplicatePhones: number
}

/**
 * Fetch duplicate phone information from the server-side API.
 * This bypasses RLS to ensure cross-sheet duplicate detection works
 * regardless of the user's Supabase permissions.
 */
export async function apiGetDuplicates(authToken?: string): Promise<DuplicatesResponse> {
  const headers: Record<string, string> = { 'Content-Type': 'application/json' }

  // If no explicit token, try to get it from the current Supabase session
  if (!authToken) {
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      authToken = sessionData.session?.access_token || undefined
    } catch {
      // Ignore session errors
    }
  }

  if (authToken) headers['X-Supabase-Auth'] = authToken

  const res = await fetch('/api/duplicates', { headers })
  if (!res.ok) {
    const data = await res.json().catch(() => ({ error: 'Unknown error' }))
    console.error('[apiGetDuplicates] Error:', data.error || res.statusText)
    return { duplicates: {}, totalLeads: 0, totalDuplicatePhones: 0 }
  }
  return res.json()
}

/**
 * Debounce map for realtime UPDATE events.
 * Key = `${table}:${id}`, Value = timer handle.
 * Prevents rapid-fire cache updates when the same lead is updated multiple
 * times in quick succession (e.g., bulk operations, concurrent users).
 *
 * Priority fields (attended, sales_name, is_archived) bypass debounce
 * so that cross-user actions like marking attendance appear instantly.
 */
const realtimeDebounceTimers: Record<string, ReturnType<typeof setTimeout>> = {}
const DEBOUNCE_MS = 50

/** Fields whose changes should bypass the debounce and fire immediately */
const PRIORITY_FIELDS = new Set([
  'attended',
  'attendance_marked_at',
  'attendance_marked_by',
  'sales_name',
  'is_archived',
  'archived_at',
])

function hasPriorityChange(
  oldRow: Record<string, unknown> | undefined,
  newRow: Record<string, unknown>
): boolean {
  if (!oldRow) return true // No old row = can't compare, treat as priority
  for (const field of PRIORITY_FIELDS) {
    if (oldRow[field] !== newRow[field]) return true
  }
  return false
}

// ===== Broadcast Channel for Cross-User Real-Time Notifications =====
// Supabase Broadcast channels are NOT subject to RLS policies,
// making them reliable for cross-user notifications even when
// postgres_changes events are blocked by Row Level Security.

/** Broadcast message format */
export interface BroadcastMessage {
  type: 'attendance' | 'assignment' | 'status' | 'archive' | 'note' | 'delete' | 'reset-attendance'
  leadId: number
  data: Partial<Lead>
  by: string       // who made the change
  byRole: string   // role of who made the change
  at: number       // timestamp (epoch ms)
}

/** Module-level reference to the broadcast channel */
let broadcastChannelRef: ReturnType<typeof supabase.channel> | null = null

/**
 * Broadcast a change to all connected clients.
 * This bypasses RLS entirely and ensures all users see important
 * changes (attendance, assignment, etc.) in real-time.
 */
export function apiBroadcastChange(message: BroadcastMessage): void {
  // Lazily create the channel if it doesn't exist yet
  if (!broadcastChannelRef) {
    broadcastChannelRef = supabase.channel('leads_changes', {
      config: { broadcast: { self: false } },
    })
  }

  broadcastChannelRef.send({
    type: 'broadcast',
    event: 'lead_change',
    payload: message,
  })
}

// ===== Toast Deduplication =====
// Prevents duplicate toast notifications when the same change is received
// via both postgres_changes AND broadcast channel.
const recentToastMap = new Map<number, number>() // leadId → timestamp
const TOAST_DEDUP_MS = 3000 // Skip duplicate toasts within 3 seconds

/**
 * Check if a toast for this lead was recently shown.
 * Returns true if the toast should be SKIPPED (duplicate).
 */
export function isDuplicateToast(leadId: number): boolean {
  const lastTime = recentToastMap.get(leadId) || 0
  const now = Date.now()
  if (now - lastTime < TOAST_DEDUP_MS) {
    return true // Skip this toast
  }
  recentToastMap.set(leadId, now)
  // Clean up old entries (older than 10 seconds)
  for (const [id, ts] of recentToastMap) {
    if (now - ts > 10000) recentToastMap.delete(id)
  }
  return false
}

export function apiSubscribeToLeads(
  callback: (payload: Record<string, unknown>) => void,
  onStatusChange?: (status: 'SUBSCRIBED' | 'CLOSED' | 'CHANNEL_ERROR' | 'TIMED_OUT') => void
) {
  const channel = supabase
    .channel('leads_changes', {
      config: { broadcast: { self: false } }, // Don't receive own broadcasts
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'leads' }, (rawPayload) => {
      const payload = rawPayload as Record<string, unknown>
      const eventType = payload.eventType as string

      // Debounce UPDATE events to avoid rapid-fire cache mutations,
      // EXCEPT for priority fields (attendance, assignment, archive) which fire immediately.
      if (eventType === 'UPDATE') {
        const newRow = payload.new as Record<string, unknown> | undefined
        const oldRow = payload.old as Record<string, unknown> | undefined
        const id = newRow?.id
        if (id) {
          // Priority changes (attendance, assignment, archive) bypass debounce
          if (hasPriorityChange(oldRow, newRow)) {
            // Clear any pending debounced update for this lead (it's now stale)
            const key = `leads:${id}`
            if (realtimeDebounceTimers[key]) {
              clearTimeout(realtimeDebounceTimers[key])
              delete realtimeDebounceTimers[key]
            }
            callback(payload)
            return
          }

          const key = `leads:${id}`
          if (realtimeDebounceTimers[key]) {
            clearTimeout(realtimeDebounceTimers[key])
          }
          realtimeDebounceTimers[key] = setTimeout(() => {
            delete realtimeDebounceTimers[key]
            callback(payload)
          }, DEBOUNCE_MS)
          return
        }
      }

      // INSERT and DELETE events pass through immediately (no debounce)
      callback(payload)
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'lead_notes' }, (payload) =>
      callback({ ...payload, table: 'lead_notes' })
    )
    .on('broadcast', { event: 'lead_change' }, (rawPayload) => {
      // Handle broadcast messages from other clients.
      // These bypass RLS and ensure cross-user notifications work reliably
      // even when postgres_changes events are blocked by Row Level Security.
      const payload = rawPayload as Record<string, unknown>
      const broadcastMsg = payload.payload as BroadcastMessage | undefined
      if (broadcastMsg) {
        console.log(`[broadcast] Received ${broadcastMsg.type} for lead ${broadcastMsg.leadId} by ${broadcastMsg.by}`)
        callback({
          eventType: 'BROADCAST',
          source: 'broadcast',
          table: 'leads',
          broadcastMessage: broadcastMsg,
        })
      }
    })
    .subscribe((status, err) => {
      // Log subscription status changes for debugging
      console.log(`[realtime] Subscription status: ${status}`, err || '')
      if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
        console.error(`[realtime] ❌ Subscription error: ${status}`, err)
      } else if (status === 'CLOSED') {
        console.warn('[realtime] ⚠️ Subscription closed')
      } else if (status === 'SUBSCRIBED') {
        console.log('[realtime] ✅ Successfully subscribed to leads channel')
      }
      // Notify the caller of status changes
      if (onStatusChange) {
        onStatusChange(status as 'SUBSCRIBED' | 'CLOSED' | 'CHANNEL_ERROR' | 'TIMED_OUT')
      }
    })

  // Store the channel reference for broadcasting
  broadcastChannelRef = channel

  return channel
}

export function apiUnsubscribe(channel: ReturnType<typeof supabase.channel>) {
  if (channel) supabase.removeChannel(channel)
}
